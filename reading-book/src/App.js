...
class App extends Component {
  ...
  createNewBook(book) {
    ...
    const url = 'http://myapi-profstream.herokuapp.com/api/449d06/books'
    fetch(url, {
      method: 'POST',
      headers: {
  ...
  deleteBook(id) {
    const url = `http://myapi-profstream.herokuapp.com/api/449d06/books/${id}`
    fetch(url, {
      method: 'DELETE',
      headers: {
        "Content-Type": "application/json"
  ...
  componentDidMount() {
    fetch('http://myapi-profstream.herokuapp.com/api/449d06/books')
      .then(response => response.json())
      .then(data => {
        console.log(data)
        this.setState({
